<template>
  ...
  onMounted(() => {
    auth.onAuthStateChanged(user => {
      if (user) {
        ...
        archiveCollection.where('user', '==', userId).onSnapshot((snapshot) => {
  ...
  Swal.fire({
    title: '¿Estás seguro de que quieres quitar esta nota archivada?',
    showCancelButton: true,
    confirmButtonText: 'Sí, quitar de archivos',
    cancelButtonText: 'No, cancelar',
  }).then((result) => {
    if (result.isConfirmed) {
      const trashRef = archiveCollection.doc(nota.id);
        ...
        if (doc.exists) {
          ...
          noteRef.set(doc.data()).then(() => {
            trashRef.delete().then(() => {
              ...
              console.error('Error al eliminar la nota de los archivos: ', error);
              Swal.fire('Error', 'Ocurrió un error al eliminar la nota de los archivos', 'error');
            });
          }).catch((error) => {
          ...
          console.log('No se encontró la nota en los archivos');
          Swal.fire('Error', 'No se encontró la nota en los archivos', 'error');
        }
      }).catch((error) => {
        console.error('Error al obtener la nota de los archivos: ', error);
        Swal.fire('Error', 'Ocurrió un error al obtener la nota de los archivos', 'error');
      });
    }
  });
}
